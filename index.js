const { MongoClient, ServerApiVersion } = require('mongodb');
const express = require('express');
const cors = require('cors');
require('dotenv').config();

const app = express();
const port = process.env.PORT || 5000;

// middleware
app.use(cors());
app.use(express.json());



//--------------------------- Start Of MongoDB Connection Code -------------------------------//

// const uri = "mongodb+srv://sadakathossain11:<db_password>@cluster1.csyfged.mongodb.net/?retryWrites=true&w=majority&appName=Cluster1";
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster1.csyfged.mongodb.net/?retryWrites=true&w=majority&appName=Cluster1`;

// Create a MongoClient with a MongoClientOptions object to set the Stable API version
const client = new MongoClient(uri, {
    serverApi: {
        version: ServerApiVersion.v1,
        strict: true,
        deprecationErrors: true,
    }
});

async function run() {
    try {
        // Connect the client to the server	(optional starting in v4.7)
        await client.connect();

        //--------------------------- Start Database Code -------------------------------//

        const database = client.db('TaskManagement').collection('AllTask')

        app.get("/allTask", async (req, res) => {
            const result = await database.find().toArray();
            res.send(result);
        });

        //--------------------------- Finished Database Code -------------------------------//



        // Send a ping to confirm a successful connection
        await client.db("admin").command({ ping: 1 });
        console.log("Pinged your deployment. You successfully connected to MongoDB!");
    } finally {
        // Ensures that the client will close when you finish/error
        // await client.close();
    }
}
run().catch(console.dir);

//--------------------------- End Of MongoDB Connection Code -------------------------------//


// get Operation
app.get("/", async (req, res) => {
    res.send("Error server is running")
});


app.listen(port, () => {
    console.log(`Server is running on port ${port}`);
});